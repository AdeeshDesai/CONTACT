#!/usr/bin/env bash
# CONTACT | task: barbed_flat | modality: vision
# usage: bash training/barbed_flat/vision.sh [seed] [hydra overrides...]
cd "$(dirname "$0")/../.."
python train.py --config-name=train_diffusion_workspace_disassembly.yaml \
    ...
    exp_name=vision_barbed_flat_50 \
    logging.project=dp_barbed_flat \
    hydra.run.dir=data/outputs/vision_barbed_flat_50/${1:-42} \
