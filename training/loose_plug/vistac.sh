#!/usr/bin/env bash
# CONTACT | task: loose_plug | modality: vistac
# usage: bash training/loose_plug/vistac.sh [seed] [hydra overrides...]
cd "$(dirname "$0")/../.."
python train.py --config-name=train_diffusion_workspace_disassembly.yaml \
    ...
    exp_name=vistac_loose_plug_50 \
    logging.project=dp_loose_plug \
    hydra.run.dir=data/outputs/vistac_loose_plug_50/${1:-42} \
