#!/usr/bin/env bash
# CONTACT | task: lidded_loose | modality: vistac
# usage: bash training/lidded_loose/vistac.sh [seed] [hydra overrides...]
cd "$(dirname "$0")/../.."
python train.py --config-name=train_diffusion_workspace_disassembly.yaml \
    ...
    exp_name=vistac_lidded_loose_50 \
    logging.project=dp_lidded_loose \
    hydra.run.dir=data/outputs/vistac_lidded_loose_50/${1:-42} \
